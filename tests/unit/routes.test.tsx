...
describe('全站路由与端到端交互', () => {
  ...
  it('CSS 渐变生成器工具页真实可用且无崩溃', () => {
    renderAt('/tools/css/gradient-generator');
    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe('CSS 渐变生成器');
    expect(screen.getByText(/实时渐变效果预览/)).toBeTruthy();
  });

  it('访问未知路径正常导向 404 页面', () => {
    renderAt('/not-exist-route');
